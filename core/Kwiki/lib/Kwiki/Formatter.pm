...
sub html_start {
    ...
    unless(ref($prev) and $prev->isa('Kwiki::Formatter::List')) {
        $self->nested_in($self->formatter_id);
        my ($last, $this) = ($self, $next);
        while(ref($this) and $this->isa('Kwiki::Formatter::List')) {
            if($this->level == 1) {
                $this->nested_in($this->formatter_id)
            } elsif($this->level <= ($last->start_level || $last->level)) {
...
sub html_end {
    ...
    if (ref($next) and $next->isa('Kwiki::Formatter::List')) {
        ...
        if ($level == 0) {
            if ($self->level == $next->level and
                    $self->nested_in ne $next->nested_in
                ) {
                $level++;
            ...
            my $prev = $self->prev_unit;
            if(ref($prev) and $prev->isa('Kwiki::Formatter::List')) {
                if (($self->nested_in eq $next->nested_in) and
                    ($self->nested_in ne $prev->nested_in)) {
                    $level++;
                    $next->start_level($next->start_level + 1);
                }
            }
...
sub html {
    ...
    return qq(<embed src="$text" WIDTH="100%" HEIGHT="250px" CONTROLS="TRUE">)
      if $text =~ /(mov|mp4)$/i;
...
sub html {
    ...
    $title = $target 
      unless $title =~ /\S/;
    return qq(<img src="$target" title="$title"/>)
      if $target =~ /(?:jpe?g|gif|png)$/i;
