...
explicitely call Date_Init.  Those functions are all done in the main
...
explicitely by the programmer before using these routines.
...
Date::Manip can have a huge impact on it's performance, depending on the
...
format, but there is a problem with days occuring in the first or last week
of the year.

The ISO-8601 representation of Jan 1, 1993 written in the YYYY-wWW-D format
is actually 1992-W53-5.  In other words, Jan 1 is treates as being in the
...
a zero, so if you are going to use Date::Manip in conjuction with these,
...
First, it requires a decription of the frequency of the event.  Examples
...
know at least one day on which the event occured.
...
(both a start and end date) as an asterix separated list:
...
If an asterix is included, values to the left of it refer to the number of
...
months.  I say approximately, because elements to the right of the asterix,
as well as any flags included in the recurrence will affect when the actual
events occur.

If no asterixes are included, then the entire recurrence is of this form.
For example,
...
Values that occur after an asterix refer to a specific value for that type
...
Values to the right of the asterix can be listed a single values, ranges
...
When the day element occurs to the right of the asterix, it can take on
multiple meanings, depending on the value of the month and week elements.
It can refer to the day of the week, day of the month, or day of the year.
Similarily, if the week element occurs to the right of the asterix, it
actually refers to the n'th time a certain day of the week occurs, either
in the month or in the year.

If the week element is non-zero and the day element is non-zero, it refers
to the day of the week.  It can be any value from 1 to 7 (negative values
-1 to -7 are also allowed). If you use the ISO 8601 convention, the first
day of the week is Monday (though Date::Manip can use any day as the start
of the week by setting the FirstDay config variable).  So, assuming that
you are using the ISO 8601 convention, the following examples illustrate
...
correct delta (ignoring daylight savings time) is returned in weeks, days,
...
making the week element (N) refer to the Nth occurence of a day of the
...
been modified to use the DateFormat config varible, so it may return
...
2nd NOTE: It has been reported to me that you can still use ParseDate
